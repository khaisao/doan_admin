...
class PrefetchRecycledViewPool(activity: Activity, coroutineScope: CoroutineScope) :
    ...
    private val viewHolderCreator =
        ViewHolderCreator(activity, coroutineScope, ::putViewFromCreator)

    override fun setViewsCount(
        viewType: Int,
        count: Int,
        holderCreator: (fakeParent: ViewGroup, viewType: Int) -> RecyclerView.ViewHolder
    ) {
        require(count > 0)
